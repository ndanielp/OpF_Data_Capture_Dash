...
def run_consents(
    dates: list[str], db_path: str | Path, workers: int = _WORKER_COUNT, logger=None
) -> int:
    """
    ...
    with mp.Manager() as mgr:
        ...
        with ProcessPoolExecutor(max_workers=n) as executor:
            futures = {
                executor.submit(_worker_run_consents, i + 1, chunk, dates, fetched_at, queue): i + 1
            ...
            while len(completed) < len(futures):
                while not queue.empty():
                    _handle_queue_msg(queue.get_nowait(), log, "Consentimentos")
    ...
    total = upsert_consents(con, all_records, fetched_at)
    con.close()

    log.info(f"Consentimentos: {total} registros inseridos/atualizados no SQLite")
    return total
        ...
        for i, receptor in enumerate(chunk, 1):
            ...
            for api_id in APIS:
                for status in STATUSES:
                    ...
                    all_records.extend(records)
                    nonzero = sum(1 for r in records if r["total"] > 0)
                    queue.put(("combo", worker_id, api_id, status, nonzero))
...
def run_api_requests(
    ...
    with mp.Manager() as mgr:
        ...
        with ProcessPoolExecutor(max_workers=n) as executor:
            futures = {
                executor.submit(
                    _worker_run_api_requests, i + 1, chunk, dates, fetched_at, queue
                ): i + 1
                for i, chunk in enumerate(chunks) if chunk
            }
            completed: set[int] = set()
            ...
            while len(completed) < len(futures):
                while not queue.empty():
                    _handle_queue_msg(queue.get_nowait(), log, "API Requests")
    ...
    con = sqlite3.connect(str(db_path))
    con.execute("PRAGMA journal_mode=WAL")
    total = upsert_api_requests(con, all_records)
    con.close()

    log.info(f"API Requests: {total} registros inseridos/atualizados no SQLite")
    return total
