...
def get_receptor_profile(institution: str, from_date: str = "2000-01-01", to_date: str = "2100-01-01"):
    ...
    for group_name, group_info in API_GROUPS.items():
        ...
        g_eco = sum(eco_totals.get(a, 0) for a in group_info["apis"])
        category_shares[group_name] = g_inst / inst_total_all if inst_total_all > 0 else 0
        tornado_data.append({"category": group_name, "label": group_name, "institution_share": category_shares[group_name], "ecosystem_share": g_eco / eco_total_all if eco_total_all > 0 else 0})
...
"""
with open('dashboard/services/of_analytics.py', 'w', encoding='utf-8') as f:
    f.write(code)
print("Updated of_analytics.py!")
