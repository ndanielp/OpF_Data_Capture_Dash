...
def get_receptor_profile(institution: str, from_date: str = "2000-01-01", to_date: str = "2100-01-01"):
    ...
    _err_valid = [x for x in all_ranks if x["err_rate"] is not None]
    eco_avg_error = sum(x["err_rate"] for x in _err_valid) / len(_err_valid) if _err_valid else 0
    ...
    err_rank = get_rank(err_sorted)
    tx_rank  = get_rank(tx_sorted)
    vol_rank = get_rank(vol_sorted)
    ...
    # Mapa Estratégico: abre conexão própria (con já foi fechada acima)
