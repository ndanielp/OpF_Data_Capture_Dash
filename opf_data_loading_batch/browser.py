"""
browser.py — Utilitários Playwright anti-detecção
===================================================
Funções e constantes para criação de browsers headless com spoof de webdriver,
navegação com retry para CloudFront 403, e manipulação de datas.

Derivado de openfinance-poc/utils.py — sem dependências Rich.
"""
...
def close_browser_clean(browser, page) -> None:
    """Limpa cookies, localStorage/sessionStorage, fecha contexto e browser."""
    try:
        ctx = page.context
        ctx.clear_cookies()
        try:
...
def goto_with_retry(
    page, url: str, selector: str, max_retries: int = 3, logger=None
) -> None:
    """
    Navega para URL e aguarda selector aparecer, com até max_retries tentativas.
    - 403 CloudFront: backoff longo (60s, 120s).
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp and resp.status == 403:
                ...
                wait_s = 60 * (attempt + 1)
                log.warning(f"403 CloudFront, aguardando {wait_s}s (tentativa {attempt + 1})")
            ...
            if attempt == max_retries - 1:
                raise
            log.warning(f"Retry {attempt + 1}/{max_retries}: {exc}")
